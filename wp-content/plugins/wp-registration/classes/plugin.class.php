	...
	function login_redirect( $redirect_to, $request, $user ) {
		...
		if ( isset( $user->roles ) && is_array( $user->roles ) ) {
			...
			if ( in_array( 'administrator', $user->roles ) ) {
				...
				//print_r($user->roles);
				$role_array = $this -> get_option('_user_redirect');
				$user_redirect = explode("\n", $role_array);
				foreach ($user_redirect as $single_role)
				{
					$userrole = explode(":", $single_role);
					if ( $user->roles[0] == $userrole[0] ){
						$redirecturl = $userrole[1];
						break;
					}
				}
				//if ( in_array( $user->roles[0], $user->roles ) ) {}
				//$userrole = explode(":", $user_redirect[0]);
				return $redirecturl;
				//print_r($user_redirect);
				//return home_url();
			}
		} else {
			return $redirect_to;
